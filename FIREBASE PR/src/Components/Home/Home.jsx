...
import { selectUserInfo } from '../../Services/selectors'
...
const Home = () => {
  ...
  const { products = [], loading, error, filters = {} } = useSelector(state => state.productList || {})
  ...
  const getSortedProducts = () => {
    const productsToSort = Array.isArray(products) ? [...products] : []
  ...
  // FIXED: Add safe array slicing with default empty arrays
  const featuredProducts = Array.isArray(products) ? products.slice(0, 8) : []
  const smartphoneDeals = Array.isArray(products) ? products.filter(p => p.category === 'electronics').slice(0, 6) : []
  const topDeals = Array.isArray(products) ? products.slice(0, 6) : []
  const fashionDeals = Array.isArray(products) ? products.filter(p => p.category === 'fashion').slice(0, 4) : []
      ...
      <div className="container-fluid mt-4">
        <div className="row">
          ...
          <div className="col-12">
            {!filters.searchQuery && (!filters.category || filters.category === 'all') && (
              <>
                <Section
                ...
                {loading ? (
                  ...
                  <div className="row">
                    {sortedProducts.map((product, index) => (
                      <div key={product.id || index} className="col-xl-3 col-lg-4 col-md-6 mb-4">
                        <ProductCard
                          product={product}
