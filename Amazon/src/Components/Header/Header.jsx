...
import { setSearchQuery, getProducts } from '../../Services/actions/productActions'
    ...
    <>
      ...
      <nav className="navbar navbar-dark bg-amazon-dark sticky-top" style={{ height: '80px', width: '100%' }}>
        <div className="container-fluid d-flex align-items-center" style={{ maxWidth: '1580px', margin: '0 auto' }}>
          ...
          {/* Location */}
          <div
            className="me-3 text-white cursor-pointer d-flex align-items-center"
            role="button"
          ...
          >
            ...
            <div>
              ...
              <div className="fw-bold small">India 394101</div>
            </div>
          </div>
              ...
              >
                {userInfo ? (
                  <>
                    ...
                    <li><Link className="dropdown-item" to="/profile" onClick={() => setShowAccountDropdown(false)}>Your Profile</Link></li>
                    // In your Header component, add this navigation link

                    <li><Link className="dropdown-item" to="/orders" onClick={() => setShowAccountDropdown(false)}>Your Orders</Link></li>
                    {userInfo.isAdmin && (
                      <li><Link className="dropdown-item" to="/add-product" onClick={() => setShowAccountDropdown(false)}>Add Product</Link></li>
                    )}
                    <li><hr className="dropdown-divider" /></li>
                  ...
                  <>
                    <li><Link className="dropdown-item" to="/login" onClick={() => setShowAccountDropdown(false)}>Sign in</Link></li>
                    <li><Link className="dropdown-item" to="/register" onClick={() => setShowAccountDropdown(false)}>New customer? Start here</Link></li>
                  </>
                )}
              </ul>
            </div>
      ...
      <nav className="navbar navbar-dark bg-amazon-secondary" style={{ height: '52px', width: '100%' }}>
        <div
          ...
          style={{ maxWidth: '1580px', margin: '0 auto' }}
        >
